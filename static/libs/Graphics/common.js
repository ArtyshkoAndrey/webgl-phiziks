...
export class Viewer {
  constructor (element, config, viewmode) {
    this.config = config || new Config().defaultConfig()
    ...
    if (!this.viewmode) {
      this.viewmode = new ViewModes().Standard()
    }
  }
  dispose () {
  ...
  _loadContentFromString (content, contentFormat, dataReadyCallback) {
    return new Utils.Promise((complete, error) => {
      let parser = new Parser()
      if (parser) {
        console.time('parsing ' + contentFormat)
        let molecule = parser.parse(content)
  ...
  _renderMolecule (molecule) {
    ...
    return new Utils().Promise((complete, error) => {
      ...
      if (this.config.renderers) {
        ...
        let nbrenderers = this.config.renderers.length
        let p = []
        this.config.renderers.forEach((rendererName) => {
          let RendererClass = Renderer[rendererName]
          if (RendererClass) {
            let renderer = new RendererClass(this, this.context, this.config)
            p.push(renderer.render(this.molecule))
          }
        })
        Utils.Promise.all(p).then(() => {
  ...
  getMoleculeFileFormat (filename) {
    if (this.endsWith(filename, '.mol') || this.endsWith(filename, '.sdf')) {
      return 'mol'
    }
    return null
  }
  ...
  loadContentFromUrl (url, contentFormat, datareadycallback, completedcallback) {
    if (!contentFormat) {
      contentFormat = this.getMoleculeFileFormat(url)
    }
    if (!contentFormat) {
  ...
  _calculateAtomsBondsAsync (molecule) {
    ...
    let bonds = []
    let nbatoms = molecule.atoms.length
    return new Utils().runBatch(0, 300, molecule.atoms, (atom, batchindex, index) => {
      ...
      for (let i = index + 1; i < nbatoms; i++) {
        let siblingAtom = molecule.atoms[i]
        let l = new BABYLON.Vector3(atom.x, atom.y, atom.z)
        let m = new BABYLON.Vector3(siblingAtom.x, siblingAtom.y, siblingAtom.z)
        let d = BABYLON.Vector3.Distance(l, m)
  ...
  _center (molecule) {
    ...
    let shift = this._getCentroid(molecule)
    molecule.atoms.forEach((atom) => {
      atom.x -= shift.x
      atom.y -= shift.y
      atom.z -= shift.z
  ...
  static getMoleculeFileFormat (moleculeUrl) {
    return ''
  }
}
...
export class Elements {
  constructor () {
    ...
    this.elements.forEach((e) => {
      this.elementsBySymbol[e.symbol.toUpperCase()] = e
    })
    this.elementsByNumber = {}
    this.elements.forEach((e) => {
      this.elementsByNumber[e.number] = e
    })
    this.MIN_ATOM_RADIUS = Infinity
    this.MAX_ATOM_RADIUS = -Infinity
    this.elements.forEach((e) => {
      this.MIN_ATOM_RADIUS = Math.min(this.MIN_ATOM_RADIUS, e.radius)
      this.MAX_ATOM_RADIUS = Math.max(this.MAX_ATOM_RADIUS, e.radius)
    })
  }
}
  ...
  parse (content) {
    ...
    for (let i = 0, l = lines.length; i < l; i++) {
      if (lines[i].indexOf('  ') === 0) {
        let lineElements = lines[i].split(' ').filter((s) => {
          let tmp = s.trim()
          if (tmp && tmp.length) {
            return true
          } else {
        ...
        if (lineElements.length && lineElements.length >= 4) {
          let symbol = lineElements[3].trim()
          let x = this.getFloat(lineElements[0])
          let y = this.getFloat(lineElements[1])
          let z = this.getFloat(lineElements[2])
          let atomKind = new Elements().elementsBySymbol[symbol.toUpperCase()]
          if (atomKind) {
            molecule.atoms.push({
              kind: atomKind,
              x: x,
...
let asap = (typeof setImmediate === 'function' && setImmediate) || function (fn) { setTimeout(fn, 1) }
let isArray = Array.isArray || function (value) { return Object.prototype.toString.call(value) === '[object Array]' }
...
export class Utils {
  ...
  runBatch (offset, size, itemslist, itemcallback, batchname) {
    if (batchname) {
    ...
    return new Promise(function (resolve, reject) {
      asap(function () {
        let items = itemslist.slice(offset, offset + size)
        ...
        if (items.length < size) {
          ...
          self.runBatch(offset + size, size, itemslist, itemcallback, batchname).then(resolve, reject)
        }
      })
    })
  }
  handle (deferred) {
    ...
    asap(function () {
      let cb = me._state ? deferred.onFulfilled : deferred.onRejected
      if (cb === null) {
    ...
    try {
      ...
      if (newValue && (typeof newValue === 'object' || typeof newValue === 'function')) {
        ...
        if (typeof then === 'function') {
          this.doResolve(then.bind(newValue), this.resolve.bind(this), this.reject.bind(this))
          return
        }
      }
      this._state = true
...
export class Promise {
  ...
  all (fake) {
    let args = Array.prototype.slice.call(arguments.length === 1 && isArray(arguments[0]) ? arguments[0] : arguments)
    return new Promise(function (resolve, reject) {
      if (args.length === 0) {
        return resolve([])
      }
